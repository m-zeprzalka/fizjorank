    ...
    <div className="min-h-screen bg-white text-slate-900 selection:bg-blue-100">
      ...
      <header className="bg-slate-50/50 border-b border-slate-100">
        <div className="max-w-6xl mx-auto px-6 pt-20 pb-16 text-center">
          ...
          <p className="text-lg text-slate-500 mb-0 max-w-2xl mx-auto font-normal leading-relaxed">
      ...
      <section className="bg-slate-50 border-t border-slate-100">
        <div className="max-w-6xl mx-auto px-6 py-20">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
            ...
            <Link href="/baza-wiedzy" className="inline-flex items-center gap-1 text-sm font-medium text-blue-600 hover:text-blue-700 transition-colors">
              Zobacz wszystkie artykuły <ChevronRight className="w-4 h-4" />
            </Link>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {articles.map((article, idx) => (
              <Link key={idx} href={`/baza-wiedzy/${article.slug}`}
                ...
                <h3 className="text-lg font-semibold text-slate-900 mb-3 leading-snug group-hover:text-blue-600 transition-colors">{article.title}</h3>
